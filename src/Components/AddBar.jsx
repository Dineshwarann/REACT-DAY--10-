...
export default function AddBar(){
    ...
    const {name,setName,userName,setUserName,email,setEmail,street,setStreet,
        ...
        if(name && userName && email && street && suite && city && zipCode && latitude && longitude && phoneNumber && companyName && website && catchPhrase && bs){
            ...
            await fetch("https://65eb3b7743ce1641893388f2.mockapi.io/data/users",{method:"POST",body:JSON.stringify(newData),headers:{"Content-Type":"application/json",},})
   ...
   //used useEffect to clear the input fields whenever it is opened
    ...
    return(
        <div className="add-section">
            ...
            <button className="btn btn-neutral" type="submit" onClick={()=>addNewMember(name,userName,email,street,suite,city,zipCode,latitude,longitude,phoneNumber,companyName,website,catchPhrase,bs)}>ADD EMPLOYEE CARD</button>
            </div>         
        </div>
    )
}